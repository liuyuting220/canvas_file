...
export default function UpData() {
    ...
    // 设置改变文件框时，改变前的文件名
    let [lastFileName,setLastrFileName] = useState('');
    // 设置传输文件的文件
                ...
                return (
                    <div className={up_data.fileBox} key = {index}>
                        <span className={up_data.textLocation}>{item.split("\\")[item.split("\\").length - 1 ]}
                            <input type="file" accept="image/*" className={up_data.fileCheck}  onChange = {changeFile} onClick = {(e)=>{setLastrFileName(item)}}/>
                        </span>
                    </div>
                )
            })}
